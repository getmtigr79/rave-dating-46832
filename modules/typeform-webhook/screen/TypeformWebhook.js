...
const TypeformWebhook = ({
  ...
  const dispatch = useDispatch(); // Function to toggle the enable state of a form
  ...
  useEffect(() => {
    if (oauthToken) {
      setIsLoading(true);
      dispatch(getForms(oauthToken)).then(unwrapResult).then(res => {
  ...
  const TypeFormAuth = () => {
    ...
    return errors.length === 0 && <Fragment>
  ...
  return <Fragment>
      {isLoading && <Loader />}
      {oauthToken ? <View style={styles.formListContainer}>
          <Text style={styles.heading}>TypeForm</Text>
          {formList.map((form, index) => <FormItem key={index} isEnabled={form?.isEnabled} toggleSwitch={toggleSwitch} form={form} oauthToken={oauthToken} navigation={navigation} />)}
        </View> : <TypeFormAuth />}
    </Fragment>;
};
